...
const init = async () => {
  ...
  // Insert movies
  await Promise.all(
    fs.readdirSync(`${THEATER_PATH}/movies`).map(async movie => {
      const id = uuid();
  ...
  const series = await Promise.all(
    fs.readdirSync(`${THEATER_PATH}/series`).map(async serie => {
      const id = uuid();
  ...
  // Insert episodes
  await Promise.all(
    series.map(async ({ id: seriesId, path, numSeasons }) => {
      return await Promise.all(
        range(1, numSeasons + 1).map(async s => {
          const dir = `${path}/s${s}`;
          const episodes = fs.readdirSync(dir).filter(f => f !== 'subtitles');
          return await Promise.all(
            episodes.map(async (e, i) => {
              console.log(e);
              const id = uuid();
